...
class User
{
    ...
    private function createUser(array $user): Entity\User
    {
      ...
      try {
        $entityRole = $identityMap->get('Entity\Role', $role['id']);
      } catch(EmptyCacheException $e) {
        $entityRole = new Entity\Role($role['id'], $role['title'], $role['role']);

        $identityMap->add($entityRole);
      }
      
      try {
        $entityUser = $identityMap->get('Entity\User', $user['id']);
      } catch(EmptyCacheException $e) {
